...
get_latest_release() {
  fetch "https://api.github.com/repos/iamcco/sran.nvim/releases/latest" |
    grep '"tag_name":' |
    sed -E 's/.*"([^"]+)".*/\1/'
}
...
download() {
  mkdir -p bin
  cd bin
  url="https://github.com/iamcco/sran.nvim/releases/download/$tag/${1}"
...
arch=$(uname -sm)
case "${arch}" in
  "Linux x86_64") download sran-linux.tar.gz ;;
  "Linux i686") download sran-linux.tar.gz ;;
  "Darwin x86_64") download sran-macos.tar.gz ;;
